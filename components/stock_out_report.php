...
// Build WHERE clause
$where_conditions = ["th.flag = 1"]; // Stock Out flag
$params = [];
$param_types = "";
...
if (!empty($search)) {
    $where_conditions[] = "(th.Trans_Docs_No LIKE ? OR c.custName LIKE ? OR i.item_Name LIKE ? OR i.item_Code LIKE ?)";
    $search_param = "%$search%";
...
$main_query = "
    ...
    FROM trans_head th
    LEFT JOIN trans_line tl ON th.Trans_Docs_No = tl.Trans_Docs_No AND tl.flag = 1
    LEFT JOIN customers c ON th.custId = c.custId
    ...
    WHERE $where_clause
    GROUP BY th.Trans_Docs_No, th.Trans_date, th.remarks, c.custName, c.custAdd, b.branchName, u.userName
...
$stats_query = "
    SELECT 
        ...
        COUNT(DISTINCT th.custId) as unique_customers,
...
try {
    ...
    $data = [];
    $error_message = "Error loading stock out data: " . $e->getMessage();
}
...
// Get dropdown data
$customers = [];
$result = $conn->query("SELECT custId, custName FROM customers ORDER BY custName ASC");
if ($result) {
    while ($row = $result->fetch_assoc()) {
        $customers[] = $row;
    }
...
<div class="report-container">
    ...
    <div class="stats-summary">
        <div class="stats-line">
            ...
            <span class="stat-separator">|</span>
            <span class="stat-item">👥 <strong><?= number_format($stats['unique_customers'] ?? 0) ?></strong> Customers</span>
            <span class="stat-separator">|</span>
    ...
    <div class="filter-section">
        <form method="GET" class="filter-form" id="filterForm">
            <input type="hidden" name="page" value="stock_out_report">
                ...
                <div class="filter-group">
                    ...
                    <select name="customer" class="filter-select">
                        ...
                        <?php foreach ($customers as $customer): ?>
                            <option value="<?= $customer['custId'] ?>" <?= ($customer_filter == $customer['custId']) ? 'selected' : '' ?>>
    ...
    <div class="results-info">
        <div class="results-count">
            ...
            <?php if (!empty($search) || !empty($customer_filter) || !empty($item_filter)): ?>
...
function clearFilters() {
    window.location.href = 'admin.php?page=stock_out_report';
}
...
function exportToCSV() {
    ...
    a.href = url;
    a.download = 'stock_out_report_' + new Date().toISOString().split('T')[0] + '.csv';
...
function viewDocumentItems(docNo) {
    ...
    // Load items via AJAX
    fetch('components/get_document_items.php?doc_no=' + encodeURIComponent(docNo) + '&flag=1')
