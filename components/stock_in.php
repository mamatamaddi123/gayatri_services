...
// Debug: Show current user and branch values (uncomment to debug)
// echo "Debug: userName = $userName, userId = $userId, branchId = $userBranchId, branchName = $branchName<br>";
// echo "Debug: Current date = " . date('dmY') . "<br>";
// echo "Debug: Expected format = BRANCHID + USERID + DDMMYYYY + SEQUENCE<br>";
// echo "Debug: Example = " . $userBranchId . $userId . date('dmY') . "0001<br>";
...
if (isset($_GET['action']) && $_GET['action'] === 'new') {
    // Clear all stock-in related session data
    unset($_SESSION['added_items']);
    unset($_SESSION['form_data']);
    unset($_SESSION['edit_mode']);
    unset($_SESSION['editing_doc_no']);
    unset($_SESSION['supplier_details']);
...
function generateDocumentNumber($conn, $branchId, $userId) {
    ...
    try {
        // Check if doc_sequence table exists, create if not
        $tableCheck = $conn->query("SHOW TABLES LIKE 'doc_sequence'");
        if (!$tableCheck || $tableCheck->num_rows == 0) {
            // Create the table
            $createTable = "CREATE TABLE `doc_sequence` (
        ...
        if ($seqRow) {
            ...
            $nextNo = 1;
            $nextSeq = 2;
            $ins = $conn->prepare("INSERT INTO doc_sequence (branchId, seq_date, next_no) VALUES (?, ?, ?)");
        ...
        // Format: BRANCHID + USERID + DDMMYYYY + SEQUENCE (4 digits)
        $seqStr = str_pad((string)$nextNo, 4, '0', STR_PAD_LEFT);
        $docNo = $branchId . $userId . $datePart . $seqStr;
        
        // Debug output (uncomment to see actual generation)
        // echo "Debug Generate: Branch=$branchId, User=$userId, Date=$datePart, Seq=$seqStr, DocNo=$docNo<br>";
        ...
        $timestamp = time();
        $fallbackDocNo = $branchId . $userId . $datePart . substr($timestamp, -4);
        
        // Debug output (uncomment to see fallback)
        // echo "Debug Generate Fallback: Branch=$branchId, User=$userId, Date=$datePart, DocNo=$fallbackDocNo, Error=" . $e->getMessage() . "<br>";
...
if (isset($_POST['search_document'])) {
    ...
    if (!empty($doc_no)) {
        ...
        if ($existing_doc_data) {
            ...
            // Fetch and store supplier details
    ...
    if (!empty($_SESSION['added_items'])) {
        try {
            ...
            if (isset($_POST['update_document'])) {
                ...
                $stmt->bind_param("issi", $supId, $remarks, $docNo, $clientVersion);
                $stmt->execute();
                ...
                foreach ($_SESSION['added_items'] as $item) {
                    $stmt = $conn->prepare("INSERT INTO trans_line (Trans_Docs_No, item_Id, item_Code, qty, rate, total, flag) VALUES (?, ?, ?, ?, ?, ?, 0)");
                ...
                $stmt->bind_param("sisii", $docNo, $supId, $remarks, $userId, $userBranchId);
                $stmt->execute();
...
$suppliers = [];
$result = $conn->query("SELECT supId, supName, supAdd, phoneNo FROM suppliers ORDER BY supName ASC");
if ($result) {
    while ($row = $result->fetch_assoc()) {
        $suppliers[] = $row;
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['add_item'])) {
        ...
        if ($current_post !== ($_SESSION['last_post']['add_item'] ?? '')) {
            ...
            // Get supplier details and store in session
            if (!empty($supId)) {
                $stmt = $conn->prepare("SELECT supAdd, phoneNo FROM suppliers WHERE supId = ?");
                ...
                if ($supplier) {
                    ...
                    // Store supplier details in session to persist across requests
                    $_SESSION['supplier_details'] = [
            ...
            if ($item_name && $item_id) {
                ...
                // Refresh supplier details from session after adding item
                if (isset($_SESSION['supplier_details'])) {
                    $supplier_address = $_SESSION['supplier_details']['address'];
                    $supplier_phone = $_SESSION['supplier_details']['phone'];
                }
    ...
    if (isset($_POST['update_item'])) {
        ...
        // Debug: Uncomment to see if update is being called
        echo "Debug: Update item called - Index: $update_index, Code: $new_item_code, Qty: $new_qty, Price: $new_price<br>";
        ...
        if (isset($_SESSION['added_items'][$update_index])) {
            ...
            if ($_SESSION['added_items'][$update_index]['item_code'] !== $new_item_code) {
                ...
                $new_item_id = $item ? $item['item_Id'] : $_SESSION['added_items'][$update_index]['item_id'];
            } else {
                $new_item_name = $_SESSION['added_items'][$update_index]['item_name'];
                $new_item_id = $_SESSION['added_items'][$update_index]['item_id'];
            }
            
            $_SESSION['added_items'][$update_index]['item_id'] = $new_item_id;
...
if ($edit_mode) {
    ...
    try {
        // Check if doc_sequence table exists
        $tableCheck = $conn->query("SHOW TABLES LIKE 'doc_sequence'");
        if ($tableCheck && $tableCheck->num_rows > 0) {
        ...
        $seqStr = str_pad((string)$nextNo, 4, '0', STR_PAD_LEFT);
        $docNo =  $userBranchId . $userId . $datePart . $seqStr;
        
        // Debug output (uncomment to see values)
        // echo "Debug Preview: Branch=$userBranchId, User=$userId, Date=$datePart, Seq=$seqStr, DocNo=$docNo<br>";
        
    } catch (Exception $e) {
        // Fallback to timestamp-based approach
        $timestamp = time();
        $docNo = $userBranchId . $userId . $datePart . substr($timestamp, -4);
        
        // Debug output (uncomment to see fallback)
        // echo "Debug Fallback: Branch=$userBranchId, User=$userId, Date=$datePart, DocNo=$docNo, Error=" . $e->getMessage() . "<br>";
    }
}
        ...
        <div class="first-line">
            <div class="document-info">
                ...
                <!-- <div class="doc-info-group">
                    <label>Branch:</label>
                    <span class="info-value" title="<?= htmlspecialchars($branchName) ?>"><?= $userBranchId ?></span>
                </div>
                <div class="doc-info-group">
                    <label>User:</label>
                    <span class="info-value" title="<?= htmlspecialchars($userName) ?>"><?= $userId ?></span>
                </div>
                <div class="doc-action-group">
                    <a href="admin.php?page=stock_in&action=new" class="new-doc-btn" id="new-doc-btn" title="Start a new document">
                        📄 NEW
                    </a>
                </div> -->
            </div>
        ...
        <div id="document_search_section" style="display: none; margin-top: 8px; padding: 6px; background: #f5f5f5; border-radius: 3px;">
            <form method="post" id="documentSearchForm">
                ...
                <!-- <p style="margin: 3px 0 0 0; font-size: 9px; color: #666;">
                    Format: BranchID + UserID + DDMMYYYY + Sequence
                </p> -->
            </form>
        </div>
    </div>
    ...
    <div class="items-table-section">
        ...
        <div class="remarks-section">
            <form method="post" class="action-form">
                ...
                <?php if ($edit_mode): ?>
                    <a href="admin.php?page=stock_in&action=new" class="clear-btn" style="text-decoration: none; display: inline-block; padding: 10px 20px;">NEW DOCUMENT</a>
                <?php endif; ?>
            </form>
        </div>
    </div>
...
$('#supplier_select').change(function(){
    ...
    // Update hidden supplier field for item form
    $('#hidden_supplier').val($(this).val());
});

// Initialize supplier details on page load
$(document).ready(function() {
...
// Edit modal item selection is now handled by the searchable dropdown
...
document.getElementById('update-item-btn').onclick = function() {
    ...
    // Debug: Uncomment to see values being submitted
    console.log('Debug: Update values - Index:', index, 'Code:', newItemCode, 'Qty:', newQty, 'Price:', newPrice);
...
// Form validation
$(document).ready(function() {
    // Handle NEW DOCUMENT button with confirmation
    $('#new-doc-btn').click(function(e) {
        const itemCount = document.querySelectorAll('#items-tbody tr').length;
        const hasSupplier = $('#supplier_select').val();
        
        if (itemCount > 0 || hasSupplier) {
            const message = itemCount > 0 
                ? `You have ${itemCount} unsaved items. Starting a new document will lose all current data. Continue?`
                : 'You have unsaved data. Starting a new document will lose current data. Continue?';
        ...
        // Reset searchable dropdowns before navigation
